...
namespace AST {
    ...
    struct Context {
        ...
        // оголосити у поточному скоуп≥
        bool declare(const std::string& name, double value) {
            auto& cur = scopes.back();
            if (cur.count(name)) return false; // уже Ї в цьому скоуп≥
            cur[name] = value;
            return true;
        }

        // присвоњти у найближчому знайденому скоуп≥
    ...
    struct Block : Stmt {
        ...
        bool createScope = false; // <Ч Ќќ¬≈

        Block() = default;
        explicit Block(bool scoped) : createScope(scoped) {}

        void setScoped(bool v) { createScope = v; } // <Ч Ќќ¬≈
        void add(Stmt* s) { items.emplace_back(s); }

        void exec(Context& ctx) const override {
            if (createScope) ctx.push();     // <Ч Ќќ¬≈: входимо у скоуп
            for (auto& s : items) s->exec(ctx);
            if (createScope) ctx.pop();      // <Ч Ќќ¬≈: виходимо з≥ скоупу
        }
